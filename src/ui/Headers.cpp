    ...
    const std::string& imagePrecisionFormat)
{
    ...
    float binWidth = xRange / static_cast<float>(histoSettings.m_numBins);
    const float xSpeed = isIntegerType(settings.componentType()) ? 1.0f : (xRange / 1000.0f);

    if (ImGui::DragFloat("Bin width", &binWidth, xSpeed, 0.0f, xRange, imagePrecisionFormat.c_str()))
    {
        if (binWidth > 0.0)
        {
            histoSettings.m_numBins = static_cast<int>( std::ceil(xRange / binWidth) );
        }
    }
        ...
        const AllViewsRecenterType& recenterAllViews)
{
    ...
    ImGui::InputScalarN("Dimensions (vox)", ImGuiDataType_U32, glm::value_ptr(dimensions), 3, nullptr, nullptr, nullptr, ImGuiInputTextFlags_ReadOnly);
    ImGui::SameLine(); helpMarker("Matrix dimensions in voxels");

    // Spacing:
    glm::vec3 spacing = imgHeader.spacing();
    ImGui::InputScalarN("Spacing (mm)", ImGuiDataType_Float, glm::value_ptr(spacing), 3, nullptr, nullptr, "%0.6f", ImGuiInputTextFlags_ReadOnly);
    ImGui::SameLine(); helpMarker("Voxel spacing (mm)");

    // Origin:
    glm::vec3 origin = imgHeader.origin();
    ImGui::InputScalarN("Origin (mm)", ImGuiDataType_Float, glm::value_ptr(origin), 3, nullptr, nullptr, coordFormat, ImGuiInputTextFlags_ReadOnly);
    ...
    ImGui::InputScalarN("Center (mm)", ImGuiDataType_Float, glm::value_ptr(boxCenter), 3, nullptr, nullptr, coordFormat, ImGuiInputTextFlags_ReadOnly);
    ImGui::SameLine(); helpMarker("Bounding box center in Subject space (mm)");


    glm::vec3 boxSize = imgHeader.subjectBBoxSize();
    ImGui::InputScalarN("Size (mm)", ImGuiDataType_Float, glm::value_ptr(boxSize), 3, nullptr, nullptr, coordFormat, ImGuiInputTextFlags_ReadOnly);
    ...
    auto applyOverrideToAllSegsOfImage = [&imageUid, &appData]
    ...
    if (Image::ImageRepresentation::Image == image.imageRep())
    {
        ...
        ImGui::SameLine();
        helpMarker("These options override image header parameters that affect the Voxel-to-Subject transformation");
        ...
        if (ImGui::Checkbox("Set 1mm isotropic voxel spacing", &(overrides.m_useIdentityPixelSpacings)))
        {
            ...
            recenterAllViews(sk_recenterCrosshairs, sk_recenterOnCurrentCrosshairsPosition, sk_doNotResetObliqueOrientation, sk_doNotResetZoom);
        }
        ...
        if (overrides.m_originalIsOblique && ! overrides.m_useIdentityPixelDirections)
        {
            ...
            ImGui::SameLine(); helpMarker("Snap to the closest orthogonal voxel direction cosines matrix");
        }
...
//    ImGui::SameLine(); helpMarker("Image border color");
    ...
    if (ImGui::TreeNode("View Properties"))
    {
        ...
        if (showComponentSelection)
        {
            if (3 == imgHeader.numComponentsPerPixel() || 4 == imgHeader.numComponentsPerPixel())
            {
                ...
                if (imgSettings.displayImageAsColor() && 4 == imgHeader.numComponentsPerPixel())
                {
                    ...
                    ImGui::SameLine(); helpMarker("Ignore alpha component of RGBA image");
                }
            }
        ...
        ImGui::SameLine(); helpMarker("Show/hide the image on all views");
       ...
       // if (visible)
        ...
        if (imageHasFloatComponents)
        {
                                       ...
                                       windowPercentileMin, windowPercentileMax, minPercentilesFormat, maxPercentilesFormat, ImGuiSliderFlags_AlwaysClamp))
            {
                ...
                {
                    const double windowPercentileLowBumped = bumpQuantile(*image, imgSettings.activeComponent(),
                ...
                {
                    const double windowPercentileHighBumped = bumpQuantile(*image, imgSettings.activeComponent(),
...
*/
        ...
        {
            return (imgSettings.displayImageAsColor()) ? imgSettings.colorInterpolationMode() : imgSettings.interpolationMode();
        };
        ...
        if (cmap && ! imgSettings.displayImageAsColor())
        {
            ...
            const bool doQuantize = (! imgSettings.colorMapContinuous() && (ImageColorMap::InterpolationMode::Linear == cmap->interpolationMode()));
...
*/
                ...
                if (ImGui::SliderScalarN_multiComp("HSV", ImGuiDataType_S32, glm::value_ptr(hsvModsInt), 3,
                    reinterpret_cast<const void**>(hsv_mins), reinterpret_cast<const void**>(hsv_maxs), hsv_formats, 0))
                {
                    imgSettings.setColorMapHueModFactor(hsvModsInt[0] / 360.0f);
                    imgSettings.setColorMapSatModFactor(hsvModsInt[1] / 100.0f);
                    imgSettings.setColorMapValModFactor(hsvModsInt[2] / 100.0f);
    ...
    if (ImGui::TreeNode("Transformations"))
    {
        ...
        */
        ...
        if (ImGui::Checkbox("Apply##affine_T_subject", &enable_affine_T_subject) &&
             ! forceDisableInitialTxs)
        {
        ...
        if (ImGui::Checkbox("Apply##worldDef_T_affine", &enable_worldDef_T_affine) &&
             ! forceDisableInitialTxs)
        {
    ...
    if (ImGui::TreeNode("Histogram"))
    {
        ...
        {
        case ComponentType::Int8: { drawImageHistogram(static_cast<const int8_t*>(buffer), bufferSize, imgSettings, format); break; }
        case ComponentType::UInt8: { drawImageHistogram(static_cast<const uint8_t*>(buffer), bufferSize, imgSettings, format); break; }
        case ComponentType::Int16: { drawImageHistogram(static_cast<const int16_t*>(buffer), bufferSize, imgSettings, format); break; }
        case ComponentType::UInt16: { drawImageHistogram(static_cast<const uint16_t*>(buffer), bufferSize, imgSettings, format); break; }
        case ComponentType::Int32: { drawImageHistogram(static_cast<const int32_t*>(buffer), bufferSize, imgSettings, format); break; }
        case ComponentType::UInt32: { drawImageHistogram(static_cast<const uint32_t*>(buffer), bufferSize, imgSettings, format); break; }
        case ComponentType::Float32: { drawImageHistogram(static_cast<const float*>(buffer), bufferSize, imgSettings, format); break; }
        default: { break; }
        }
        ...
        const AllViewsRecenterType& recenterAllViews)
{
    ...
    static const std::string sk_fillAnnotButtonText =
            std::string(ICON_FK_PAINT_BRUSH) + std::string(" Fill segmentation");
    ...
    {
        const glm::mat3 world_T_subject_invTranspose = glm::inverseTranspose(glm::mat3{ image->transformations().worldDef_T_subject() });
        const glm::vec3 worldAnnotNormal = glm::normalize(world_T_subject_invTranspose * glm::vec3{ annot.getSubjectPlaneEquation() });
        ...
        if (viewsWithNormal.empty())
        {
            ...
            if (View* view = appData.windowData().getCurrentView(largestCurrentViewUid))
            {
                ...
                spdlog::trace("Changed view {} normal direction to {}", largestCurrentViewUid, glm::to_string(worldAnnotNormal));
            }
            else
            {
    ...
    // Header is ID'ed only by the image index.
    // ### allows the header name to change without changing its ID.
    const std::string headerName = std::to_string(imageIndex) + ") " + image->settings().displayName() + "###" + std::to_string(imageIndex);
